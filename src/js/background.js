...
Background.addSprite('dining/background.jpg',{key:'diningback',pivot:'center'})
...
Background.addSprite('geomagical/background.png',{key:'geoback',pivot:'center',scale: 0.17,x: 0.16,y: 0.56});//,pivot:{x: 0.74, y: 0.1}});//{x:0.76,y:0.2}
// Background.addMask({key:'housemaskend',maskReset:true});
Background.addSprite('house/backhole.png',{key:'house',pivot:'center'});
Background.addSprite('house/front.png',{key:'front',pivot:'center'});
...
var shotSize = 0.22;
Background.addSprite('geomagical/shot1.jpg',{key:'shot1',pivot:'center',width:shotSize,x:0.1,y:0.5});

Background.addSprite('geomagical/shot2.jpg',{key:'shot2',pivot:'center',width:shotSize,x:0.3,y:0.5});
Background.addSprite('geomagical/shot3.jpg',{key:'shot3',pivot:'center',width:shotSize,x:0.5,y:0.5});
Background.addSprite('geomagical/shot4.jpg',{key:'shot4',pivot:'center',width:shotSize,x:0.7,y:0.5});
Background.addSprite('geomagical/shot5.jpg',{key:'shot5',pivot:'center',width:shotSize,x:0.9,y:0.5});
Background.addSprite('geomagical/seam.jpg',{key:'seam',pivot:'center',position:'center'});
Background.addSprite('geomagical/semantic.jpg',{key:'semantic',pivot:'center',position:'center'});
Background.addSprite('geomagical/depth.jpg',{key:'depth',pivot:'center',position:'center'});
...
Background.addSprite('house/backholetransition.png',{key:'transition',pivot:'center',position:'center'});
Background.addSprite('house/color.png',{key:'color',pivot:'center',position:'center'});
...
Background.logic = function(){
  ...
  for(var component in AnimationData[this.currentSlug]){
    ...
    for(var key in data){
      ...
      if(key=='pivot'){
        // var deltaKey = (data[key][0]-object[key].x);
        // if(Math.abs(deltaKey)>distance) needsUpdate = true;
        // object[key].x += deltaKey/50;
        // deltaKey = (data[key][1]-object[key].y);
        // if(Math.abs(deltaKey)>distance) needsUpdate = true;
        // object[key].y += deltaKey/50;

        if(object.final.pivot==undefined) object.final.pivot = {};
        object.final.pivot.x = data.pivot.x;
        object.final.pivot.y = data.pivot.y;
      }else{
        // var deltaKey = (data[key]-object[key]);
        // if(Math.abs(deltaKey)>distance) needsUpdate = true;
        // object[key] += deltaKey/50;
        if(key[0]=='_'){
          var okey = key;
          key = key.substring(1);
          // console.log(key)
    ...
    // if(component=='house') console.log(needsUpdate);
    // object.needsUpdate = needsUpdate;
  }
...
// var renderListDiv = document.getElementById('renders');
// if(renderListDiv){
//   Background.postRender = function(){
//     renderListDiv.innerHTML = this.renderlist;
//   }
// }
